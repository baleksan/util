...
/**
 * Produces 32-bit hash for hash table lookup. <br/>
 * <p/>
 * <pre>
 ...
 *      Dr. Dobbs Article</a> <p>
 *      Lifted with modifications from the hadoop/commons project.
 *      </p>
 */
public class JenkinsHash extends Hash {
    ...
    private static JenkinsHash singleton = new JenkinsHash();

    /**
     * Singleton accessor method.
     *
     * @return global singleton hash instance.
     */
    public static Hash getInstance() {
        return singleton;
    }

    public static JenkinsHash get64BitHash() {
        return singleton;
    }
    ...
    /**
     * taken from hashlittle() -- hash a variable-length key into a 32-bit
     * value.
     *
     ...
     */
    private static final long INIT_SKEW = 0x00000000deadbeefL;
    private static final int MAGIC_12 = 12;
    private static final int MAGIC_19 = 19;
    ...
    public int hash(final byte[] key, final int nbytes, final int initval) {
        ...
        a = b = c = (INIT_SKEW + length + initval) & INT_MASK;
        for (; length > MAGIC_12; offset += MAGIC_12, length -= MAGIC_12) {
            a = (a + (key[offset + 0] & BYTE_MASK)) & INT_MASK;
            a = (a + (((key[offset + 2] & BYTE_MASK) << SHIFT_2B) & INT_MASK)) & INT_MASK;
            a = (a + (((key[offset + 1] & BYTE_MASK) << SHIFT_1B) & INT_MASK)) & INT_MASK;
            a = (a + (((key[offset + 3] & BYTE_MASK) << SHIFT_3B) & INT_MASK)) & INT_MASK;
            b = (b + (key[offset + 4] & BYTE_MASK)) & INT_MASK;
            b = (b + (((key[offset + 5] & BYTE_MASK) << SHIFT_1B) & INT_MASK)) & INT_MASK;
            b = (b + (((key[offset + 6] & BYTE_MASK) << SHIFT_2B) & INT_MASK)) & INT_MASK;
            b = (b + (((key[offset + 7] & BYTE_MASK) << SHIFT_3B) & INT_MASK)) & INT_MASK;
            c = (c + (key[offset + 8] & BYTE_MASK)) & INT_MASK;
            c = (c + (((key[offset + 9] & BYTE_MASK) << SHIFT_1B) & INT_MASK)) & INT_MASK;
            c = (c + (((key[offset + 10] & BYTE_MASK) << SHIFT_2B) & INT_MASK)) & INT_MASK;
            c = (c + (((key[offset + 11] & BYTE_MASK) << SHIFT_3B) & INT_MASK)) & INT_MASK;
        ...
        c ^= b;
        c = (c - rot(b, MAGIC_24)) & INT_MASK;

        return (int) (c & INT_MASK);
    }

    public long hash64bit(final byte[] bytes) {
        return hash64bit(bytes, bytes.length, -1);
    }

    @SuppressWarnings("fallthrough")
    public long hash64bit(final byte[] key, final int nbytes, final int initval) {
        int length = nbytes;
        int offset = 0;
        long a, b, c;
        a = b = c = INIT_SKEW + length + initval;
        for (; length > MAGIC_12; offset += MAGIC_12, length -= MAGIC_12) {
            a = (a + (key[offset + 0] & BYTE_MASK));
            a = (a + (((key[offset + 2] & BYTE_MASK) << SHIFT_2B)));
            a = (a + (((key[offset + 1] & BYTE_MASK) << SHIFT_1B)));
            a = (a + (((key[offset + 3] & BYTE_MASK) << SHIFT_3B)));
            b = (b + (key[offset + 4] & BYTE_MASK));
            b = (b + (((key[offset + 5] & BYTE_MASK) << SHIFT_1B)));
            b = (b + (((key[offset + 6] & BYTE_MASK) << SHIFT_2B)));
            b = (b + (((key[offset + 7] & BYTE_MASK) << SHIFT_3B)));
            c = (c + (key[offset + 8] & BYTE_MASK));
            c = (c + (((key[offset + 9] & BYTE_MASK) << SHIFT_1B)));
            c = (c + (((key[offset + 10] & BYTE_MASK) << SHIFT_2B)));
            c = (c + (((key[offset + 11] & BYTE_MASK) << SHIFT_3B)));
